...
require_once dirname(dirname(dirname(__FILE__)))."{$PS}config{$PS}config.php";*/
...
class Functions_adm extends Config {
	...
	public function trova_ordine($tabella, $id_rife="", $rife=""){
		...
		if(!$id_rife || !$rife) $wer = "";
		else  $wer = "where $id_rife='".$this->clean_acc($rife)."'";
	...
	public function trova_ordine2($tabella, $id_rife="", $rife="", $id_rife2="", $rife2="", $campo="ordine"){
		...
		else  $wer2 = " and $id_rife2='".$this->clean_acc($rife2)."'";
	...
	public function clean_acc($str_in)
	{
		...
		$str_out = str_replace("�", "&egrave;", $str_out);
		$str_out = str_replace("�", "&eacute;", $str_out);
		$str_out = str_replace("�", "&agrave;", $str_out);
		$str_out = str_replace("�", "&igrave;'", $str_out);
		$str_out = str_replace("�", "&ograve;", $str_out);
		$str_out = str_replace("�", "&ugrave;", $str_out);	
	...
	public function scrivi_img($nomef,$file, $direi="")
	{
		...
		if(file_exists("$dire/$nome")){
			while((file_exists("$dire/$nome"))==true){
				$exts = explode(".", $nome);
	...
	/*public function thumbjpg( $larg ,  $file ,$filename, $dir_imm="", $start="" )
	{
		...
		if($larg>0)
		{
			...
			imagecopyresized($dst_img,$src_img,0,0,0,0,$larg, $alte , imagesx($src_img),imagesy($src_img));
			imageantialias ( $dst_img, TRUE);

			if(file_exists( $dir_pic."/$start_file". $filename)){
				unlink( $dir_pic."/$start_file". $filename);
			}
			imagejpeg($dst_img, $dir_pic."/".$start_file.$filename, 90);	
		}		
	}*/
	...
	public function thumbjpg( $larg ,  $file ,$filename, $dir_imm="", $start="" )
	{
		...
		if($larg > 0)
		{
			...
			// Creazione di un'immagine con canale alfa per migliorare la qualit�
			$dst_img = imagecreatetruecolor($larg, $alte); 

			// Migliora la qualit� della miniatura
			imageinterlace($dst_img, true); // Abilita il progressive JPEG per un migliore rendering
			...
			// Salva l'immagine con qualit� 90
			imagejpeg($dst_img, $dir_pic."/".$start_file.$filename, 90);
	...
	public function thumbjpg_new( $larg ,  $file ,$filename, $dir_imm="")
	{
		...
		if($larg>0)
		{
			...
			imagejpeg($dst_img, $dir_pic."/".$filename, 90);	
		}		
	}
	
	public function thumbjpg_inv( $alte ,  $file ,$filename, $dir_imm="")
	{
		global $HTTP_HOST ;
		...
		if($alte>0)
		{
			...
			$larg = ($alte * imagesx($src_img)) /  imagesy($src_img) ;
			$larg = round($larg);
	...
	*/
	public function inserisci_campi ($tabella , $arr_no="no" ,  $arr_thumb="no", $dir_imm="", $dir_fil="", $max_w_img=1920 ){
		...
		foreach($_POST AS $k=>$v){
			if(isset($v)) $value = $v; else $value ="";
			if(isset($k)) $key = $k; else $key ="";
			
			if( !isset($arr_no[$key]) && $value!=""){
				$campi['nome'] .= " $key,";
				//$campi['valore'] .= " \"".$this->clean_acc($value)."\" ,";
				$campi['valore'] .= " :$key,";
				$params[':'.$key] = $value;
			}
		}
			...
			if($dim!=0)
			{
				...
				*/
				...
				if($finale=="jpg" || $finale=="jpeg" || $finale=="gif" || $finale=="png")
				{
					...
					if ($finale === 'jpg' || $finale === 'jpeg') {
						$this->compressJpeg("$dir_img/$nome_file", 82);
					}
					...
					*/
					...
					else if ($larghezza_gr > $max_w_img && ($finale== "jpeg" || $finale=="jpg")) 
					{
						...
						*/
						if(file_exists("$dir_img/$nome_file")) /* sicuramente � cos�, l'ho appena caricata!!! */
						{
							...
							while( file_exists("$dir_img/$nome_file") )
							{
							...
							$this->thumbjpg( $max_w_img ,  $tmp_file ,$nome_file, $dir_img, "RED_");
							$nome_file = "RED_$nome_file";
		...
		if(!$risu)
		{
			...
			$id_ins = $open_connection->connection->lastInsertId();
		}
		return $id_ins;
	}
	...
	***********************************************
	*/
	public function modifica_campi( $tabella, $id_rec ,$arr_no="no", $arr_thumb="no", $dir_imm="", $dir_fil="", $max_w_img=1920, $var_id="id" )
	{
		$open_connection = new dbnew();
		...
		$query = "select * from $tabella where $var_id='$id_rec'";
		$risu    = $open_connection->connection->query($query);
		...
		if($num_campi>0){
			...
			foreach($_POST AS $k=>$v){
				...
				$key =   $k;
				if( !isset($arr_no[$key]) && $value!=""){
					//$stringa .= " $key=\"".$this->clean_acc($value)."\" ,";
					//$campi['nome'] .= " $key,";
					$stringa .= " ".$key." = :".$key.",";
					$params[':'.$key] = $value;
				}
			...
			if (empty($_POST['correlati']) && str_contains($tabella, 'prodotti')) {
					$stringa .= " correlati = :correlati,";
					$params[':correlati'] = "";			
			}
				...
				if($dim!=0)
				{
					...
					{			
						...
						else if($larghezza_gr>$max_w_img && ($finale=="jpg" || $finale=="jpeg" ))
						{
							...
							prima devo decidere il nome
							*/
							if(file_exists("$dir_img/$nome_file")) 
							{
								/* e questo � il nome del file grande appena caricato */
								$big_file = "$dir_img/$nome_file";
								/* scelgo un altro nome per l'immagine rimpicciolita */
					...
					*/
					if($vecchio_file !="")
					{
						...
						if(file_exists("$dir_old/$vecchio_file"))unlink("$dir_old/$vecchio_file");
						if(file_exists("$dir_old/s_$vecchio_file"))unlink("$dir_old/s_$vecchio_file");
					}
			...
			$query = "update $tabella set $stringa where $var_id='$id_rec' ";
			$risu = $open_connection->connection->prepare($query);
			$risu->execute($params);
			//$risu = $open_connection->connection->query($query);
			if(!$risu)
	...
	*/
	public function cancella_record_foto($tabella , $valore , $campo="id", $cart="")
	{
		...
		$foto_nome = $row_img['foto'] ?? null;
		
		if($foto_nome){
			if(file_exists("$cartella/$foto_nome"))unlink("$cartella/$foto_nome");
			if(file_exists("$cartella/s_".$foto_nome))unlink("$cartella/s_".$foto_nome);
		}
	...
	public function campo_ins($titolo, $campo_nome, $tipo_campo, $nome_array="no", $style="", $value="", $field = "", $dir_imm = "", $dir_fil = "")
	{
		...
		if($tipo_campo == 2){
			...
			<div class=\"mws-form-row\">
				<label class=\"mws-form-label\">$titolo</label>
				<div class=\"mws-form-item\">
					<input name=\"$campo_nome\" type=\"text\" class=\"small\" style=\"$stile\" value=\"$value\" id=\"$fie\">
				</div>
			</div>
			";
		}
			...
			<div class=\"mws-form-row\">
				...
				<div class=\"mws-form-item\">
					...
					for($x=0;$x<$count_arr;$x++){
						if($x==0) reset($nome_array);
						
						$arr_data = each($nome_array);
						$kiave  = $arr_data[0];
						$valore = $arr_data[1];
						$sel ="";
						if($value==$kiave)
		...
		*/
			...
			for($x=0;$x<$num_mesi;$x++){
				if($x==0) reset($this->mega_mesi);
				
				$arr_data = each($this->mega_mesi);
				$kiave  = $arr_data[0];
				$valore = $arr_data[1];
			...
			for($x=0;$x<$num_anni;$x++){
				if($x==0) reset($this->mega_anni);
				
				
				$arr_data = each($this->mega_anni);
				$kiave  = $arr_data[0];
				$valore = $arr_data[1];
	...
	public function campo_mod($titolo , $campo_nome , $value, $tipo_campo , $nome_array="no", $att_cmd, $att_rec, $style="", $field="",$dir_imm="", $dir_fil="", $no_canc="", $readonly="0"){
		...
		if($tipo_campo == 1){
			...
			<div class=\"mws-form-row\">
				...
				<div class=\"mws-form-item\">
					<input name=\"$campo_nome\" type=\"text\" class=\"medium\" value=\"$value\" id=\"$fie\"";
					if($stile!="") echo " style=\"$stile\"";
					if($readonly==1) echo " readonly=\"readonly\"";
					echo " >";
					if($value!="" && $noc==0)
					{
					echo"<a href=\"admin.php?cmd=$att_cmd&id_rec=$att_rec&campocanc=$campo_nome\" class=\"testo10\" style=\"color:#333; text-decortion:none;\"> <i class=\"fa fa-eraser\" aria-hidden=\"true\"></i></a>
					";
					}
				echo "</div>
			</div>
		...
		{
			$src = "";
			if ($value !== "" && str_contains($dir_img, 'prodotti') && function_exists('productImageUrl')) {
				$src = productImageUrl($value, 'thumb');
			} elseif ($value !== "" && file_exists("$dir_img/s_$value")) {
				$src = "$dir_img/s_$value";
			} elseif ($value !== "" && file_exists("$dir_img/$value")) {
				$src = "$dir_img/$value";
			}
			...
			<div class=\"mws-form-row\">
				...
				if(isset($src) && $src!="")
				{
					echo "<img  style=\"margin-bottom:5px; width:200px;\" src=\"$src\" border=\"0\" align=\"absmiddle\">";
					if($value!="" && $noc==0)
					{
					echo"<a href=\"admin.php?cmd=$att_cmd&id_rec=$att_rec&campocanc=$campo_nome\" class=\"testo10\" style=\"color:#333; text-decortion:none;\"> <i class=\"fa fa-eraser\" aria-hidden=\"true\"></i></a>
					";
			...
			<div class=\"mws-form-row\">
				<label class=\"mws-form-label\">$titolo</label>
				<div class=\"mws-form-item\">
					<a href=\"$dir_files/$value\" target=\"_blank\" class=\"$stile\">Documento attuale : <b>$value</b></a>";
				if($value!="" && $noc==0){
				echo"<br><a href=\"admin.php?cmd=$att_cmd&id_rec=$att_rec&campocanc=$campo_nome\" class=\"testo10\" > cancella documento</a>
				";}
				echo"<br>
				<input name=\"$campo_nome\" type=\"file\" class=\"medium\" size=\"60\" id=\"$fie\">
				</div>
			...
			<div class=\"mws-form-row\">
				...
				foreach($nome_array AS $k=>$v){
					$kiave  = $k;
					$valore = $v;
					if($value==$kiave) echo "<option value=\"$kiave\" selected=\"selected\">$valore</option>";
					else echo "<option value=\"$kiave\">$valore</option>";
				}
		...
		*/
			...
			foreach($this->mega_mesi AS $k=>$v){
				$kiave  = $k;
				$valore = $v;
				if($mesi_att==$kiave)echo "<option value=\"$kiave\" selected=\"selected\">$valore</option>\n";
				else echo "<option value=\"$kiave\" >$valore</option>\n";
			}
			echo "</select>&nbsp;&nbsp;";
			...
			foreach($this->mega_anni AS $k=>$v){
				$kiave  = $k;
				$valore = $v;
				if($anni_att==$valore)echo "<option value=\"$kiave\" selected=\"selected\">$valore</option>\n";
				else echo "<option value=\"$kiave\">$valore</option>\n";
			}
			echo "</select>";
	...
	*/
	...
	public function creaImmagineRidimensionata($percorsoImmagine, $larghezzaNuova = "200") {
		...
		list($larghezzaOriginale, $altezzaOriginale, $tipo) = getimagesize($percorsoImmagine);

		// Calcola la nuova altezza mantenendo la proporzione
		//$larghezzaNuova = 200;
		$altezzaNuova = ($altezzaOriginale / $larghezzaOriginale) * $larghezzaNuova;

		// Crea una nuova immagine a seconda del tipo di immagine
		switch ($tipo) {
			case IMAGETYPE_JPEG:
				$immagineOriginale = imagecreatefromjpeg($percorsoImmagine);
				break;
			case IMAGETYPE_PNG:
				$immagineOriginale = imagecreatefrompng($percorsoImmagine);
				break;
			case IMAGETYPE_GIF:
				$immagineOriginale = imagecreatefromgif($percorsoImmagine);
				break;
			default:
		...
		$immagineRidimensionata = imagecreatetruecolor($larghezzaNuova, $altezzaNuova);

		// Mantieni trasparenza se � un PNG o GIF
		if ($tipo == IMAGETYPE_PNG || $tipo == IMAGETYPE_GIF) {
			imagealphablending($immagineRidimensionata, false);
			imagesavealpha($immagineRidimensionata, true);
			$trasparente = imagecolorallocatealpha($immagineRidimensionata, 0, 0, 0, 127);
			imagefilledrectangle($immagineRidimensionata, 0, 0, $larghezzaNuova, $altezzaNuova, $trasparente);
		}

		// Copia l'immagine originale ridimensionandola
		imagecopyresampled($immagineRidimensionata, $immagineOriginale, 0, 0, 0, 0, $larghezzaNuova, $altezzaNuova, $larghezzaOriginale, $altezzaOriginale);

		// Salva l'immagine ridimensionata con prefisso "xs_"
		$percorsoNuovo = dirname($percorsoImmagine) . '/xs_' . basename($percorsoImmagine);

		switch ($tipo) {
			case IMAGETYPE_JPEG:
				imagejpeg($immagineRidimensionata, $percorsoNuovo, 90);
				break;
			case IMAGETYPE_PNG:
				imagepng($immagineRidimensionata, $percorsoNuovo);
				break;
			case IMAGETYPE_GIF:
				imagegif($immagineRidimensionata, $percorsoNuovo);
				break;
		}
		...
		return "Immagine ridimensionata salvata come: " . $percorsoNuovo;
	}

	private function compressJpeg(string $path, int $quality = 82): void {
