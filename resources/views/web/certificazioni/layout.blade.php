                ...
                <div class="catalog-sidebar">
                    <div class="catalog-sidebar-header">
                        <h3 class="pt-sans-bold" style="margin:0; font-family:'Inria Serif'; font-style:italic; font-size:30px;">
                            {{ __('menu.certificazione') }}
                        </h3>
                    </div>
                    <div class="catalog-sidebar-body">
                        @include('web.certificazioni.partials.sidebar-nav')
                    </div>
                </div>
            ...
            <div class="catalog-main">
                <div class="catalog-breadcrumb">
                    <h1 class="catalog-page-title" style="font-size:50px;">
                        {{ __("certifications.{$certKey}.title") }}
                    </h1>
                    <ul class="breadcrumb-list">
                        <li><a href="{{ locale_route('home') }}" title="{{ config('app.name') }}">{{ __('catalog.home') }}</a></li>
                        <li><i class="fa fa-angle-double-right" aria-hidden="true"></i></li>
                        <li><a href="{{ locale_route('certifications.online') }}">{{ __('menu.certificazione') }}</a></li>
                        <li><i class="fa fa-angle-double-right" aria-hidden="true"></i></li>
                        <li><span class="pt-sans">{{ __("certifications.{$certKey}.title") }}</span></li>
                    </ul>
                </div>

                <div class="catalog-description small-section">
                    @hasSection('cert-content')
                        @yield('cert-content')
                    @else
                        @include('web.certificazioni.partials.page-content', ['certKey' => $certKey])
                    @endif
                </div>
            </div>
        </div>
